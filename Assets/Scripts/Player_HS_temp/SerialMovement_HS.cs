...
public class SerialMovement_HS : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Players[top].AddComponent<PlayerLaunch_HS>();
        topPlayer = Players[top].GetComponent<PlayerLaunch_HS>();
    ...
    public void OnTriggerstar(Collider2D other)
    {
        if (!levelManager.isLaunching)
        {
            ...
            Players[top].gameObject.AddComponent<PlayerLaunch_HS>();
            topPlayer = Players[top].GetComponent<PlayerLaunch_HS>();
            camera.Player = Players[top];
        }
        else
